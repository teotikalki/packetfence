...
information, please refer to our website https://packetfence.org/about/supported_switches_and_aps.html
...
The other way to support Roaming is to enable snmp trap in the AeroHIVE configuration to PacketFence server. PacketFence will recognize the ahConnectionChangeEvent and will change the location of the node in his base.
...
* Go in the section *Captive Web Portal Login Page Settings* set the *Login URL* to http://<your_captive_portal_ip>/AeroHIVE::AP and *Password Encryption* to *No Encryption*
* Use the *User Profiles* to define your web authentication VLAN
* In PacketFence configuration make sure to unselect every *Roles by* in the tab *Roles* while adding the switch, also please select the switch type *AeroHIVE AP*
...
In order to configure VLAN enforcement on the Unifi controller, you need first to configure a RADIUS profile, then next an open and a secure Wireless network

Important : You cannot re-use a VLAN ID for dynamic VLAN if it is set as a static value for another SSID on the same AP. So, if I have a SSID set to use VLAN 10, I cannot use VLAN ID 10 for RADIUS controlled VLAN users as those users will not get an IP.
...
image::docs/images/unifi-open.png[scaledwidth="100%",alt="Unifi Open SSID"]

Secure SSID
+++++++++++

There is a particularity when you want to be able to deauthenticate a device when this one is connected on the secure SSID.

By default pmksa caching is enable and it occur than even if you send a deauthentication order to the controller, you will not have any new radius request.

To disable this cache you will need to create a file on the controller.

First verify the site where your access point is managed, to do that, under the administration interface of the controller, select the appropriate "Current Site" then in the URI check the value just after site.

(in my case the uri is https://192.168.1.6:8443/manage/site/4j4ee7x4/ so the site id is 4j4ee7x4)

Next connect on the controller via ssh and go in /usr/lib/unifi/data/sites/4j4ee7x4 (replace 4j4ee7x4 by your site id) and create a file config.properties that contain:

 config.system_cfg.1=aaa.1.auth_cache=disabled
 config.system_cfg.2=aaa.4.auth_cache=disabled

1 and 4 are the profil id configured on the access point, to be sure to have the correct id, you can connect on the access point via ssh or via the debug terminal and do:
...
You can see that PacketFence-Secure appear in 1 and 4 (2 radios).

Complicate isn't it !, feel free to ask the support directly in the administration interface there: https://community.ubnt.com/t5/UniFi-Wireless/Feature-request-disable-pmksa-caching/m-p/2112479#M257628

Next you will need to create a secure profile:
